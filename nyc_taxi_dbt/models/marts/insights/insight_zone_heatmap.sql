...
select
    ...
    percent_rank() over (order by coalesce(p.pickup_count, 0)) as pickup_percentile,
    case when percent_rank() over (order by coalesce(p.pickup_count, 0)) >= 0.95 then 'Very Hot' when percent_rank() over (order by coalesce(p.pickup_count, 0)) >= 0.80 then 'Hot' when percent_rank() over (order by coalesce(p.pickup_count, 0)) >= 0.50 then 'Warm' else 'Cold' end as heat_level,
